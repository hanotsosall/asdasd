...
initJSON(USERS_FILE, [{ id: 1, username: 'admin', password: adminHash, role: 'admin', email: 'admin@steamfall.com', avatar: 'https://i.pravatar.cc/150?img=1', createdAt: new Date().toISOString(), lastSeen: new Date().toISOString(), banned: false }]);
initJSON(GAMES_FILE, [
...
app.post('/api/import', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    const developer = $('.developer').text() || 'Unknown';
    const releaseDate = new Date().toISOString().split('T')[0];
    const screenshots = ['https://picsum.photos/id/100/800/450'];
    res.json({ title, genre: 'Action', description, size, magnet, developer, releaseDate, screenshots, tags: ['import'] });
...
const storage = multer.diskStorage({ destination: TORRENTS_DIR, filename: (req, file, cb) => cb(null, Date.now() + '-' + file.originalname) });
